...
class CircleHandler:
    ...
    def detect_circles(self, params: dict | None = None) -> None:
        ...
        if params is None:
            params = dict(
                dp=1, minDist=50, param1=20, param2=100, minRadius=70, maxRadius=100
            )
    ...
    def draw_circles(self) -> np.ndarray | None:
        ...
        if len(circles[0]) != 1:
            logger.error(
                "Please provide an image with exactly one reference coin. The image appears to be ambiguous."
            )
            raise ValueError(
                "Please provide an image with exactly one reference coin. The image appears to be ambiguous."
            )
